...
//no chain of comments  ???helper > plugin 
...
//TODO: add comment to comment > parent id from params
export async function addComment(request, response){
  ...
  try{
    ...
    await post.save({session}); //not .session(session)
...
export async function editComment(request, response){
  ...
  try {
    const comment = await Comment.findByIdAndUpdate(commentId, payload, {new: true}); 
    if(!comment)
      return response.status(404).json({message: `Could NOT find and update comment ${commentId}`}); 
